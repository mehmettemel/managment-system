...
import { processClassPayment, calculateNewPaymentDate } from '@/actions/payments'
...
export function ClassPaymentModal({
  ...
  useEffect(() => {
    if (selectedClassId) {
      const mc = memberClasses.find(c => c.class_id.toString() === selectedClassId)
      if (mc) setAmount(mc.price || 0)
    }
  ...
  const selectedClass = memberClasses.find(c => c.class_id.toString() === selectedClassId)
      ...
      <Stack>
        ...
        {newDatePreview && (
          <Alert variant="light" color="grape" icon={<IconCalendar />}>
            <Text size="sm">
              Yeni ödeme tarihi: <strong>{formatDate(newDatePreview)}</strong> olacaktır.
            </Text>
          </Alert>
        )}
