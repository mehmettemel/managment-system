...
export function DanceTypesTable({ data }: { data: DanceType[] }) {
  ...
  const form = useForm({
    ...
    validate: {
      name: (value) =>
        value.trim().length < 2 ? 'En az 2 karakter olmalı' : null,
    },
  });
  ...
  const handleDelete = (id: number) => {
    modals.openConfirmModal({
      title: 'Silme Onayı',
      children: (
        <Text size="sm">Bu dans türünü silmek istediğinize emin misiniz?</Text>
      ),
  ...
  return (
    <Stack>
      ...
      <Modal
        opened={opened}
        onClose={handleClose}
        title={editingType ? 'Düzenle' : 'Yeni Ekle'}
        centered
      >
