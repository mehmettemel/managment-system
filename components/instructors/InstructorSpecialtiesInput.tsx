...
export function InstructorSpecialtiesInput({ value, onChange }: Props) {
  ...
  const availableTypes = types.filter(
    (t) => !value.some((v) => v.dance_type_id === t.id)
  );
  const selectData = availableTypes.map((t) => ({
    value: t.id.toString(),
    label: t.name,
  }));
  ...
  return (
    <Stack gap="xs">
      <Text fz="sm" fw={500}>
        Uzmanlık Alanları ve Komisyon Oranları
      </Text>
              ...
              <Table.Tr key={item.dance_type_id}>
                ...
                <Table.Td w={40}>
                  <ActionIcon
                    ...
                    variant="subtle"
                    size="sm"
                    onClick={() => handleDelete(item.dance_type_id)}
                  >
                    <IconTrash size={14} />
                  </ActionIcon>
                </Table.Td>
