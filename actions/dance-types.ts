 ...
 * Create a dance type
 */
export async function createDanceType(name: string): Promise<ApiResponse<DanceType>> {
    try {
        const supabase = await createClient()
 ...
 * Update a dance type
 */
export async function updateDanceType(id: number, name: string): Promise<ApiResponse<DanceType>> {
    try {
        const supabase = await createClient()
         const slug = name.toLowerCase().replace(/ /g, '-').replace(/[^\w-]+/g, '')
 ...
 * Delete a dance type
 */
export async function deleteDanceType(id: number): Promise<ApiResponse<boolean>> {
    try {
        const supabase = await createClient()
