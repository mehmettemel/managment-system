...
import { logError, errorResponse, successResponse } from '@/utils/response-helpers'
 ...
 */
export async function getPaymentStatusStats(): Promise<ApiResponse<{ name: string; value: number; color: string }[]>> {
    try {
        // Parallel fetch
 ...
 */
export async function getLessonPopularityStats(): Promise<ApiResponse<{ name: string; value: number }[]>> {
    try {
        const supabase = await createClient()
        
        const { data: memberClasses, error: mcError } = await supabase
            .from('member_classes')
            .select(`
                class_id,
                classes (
